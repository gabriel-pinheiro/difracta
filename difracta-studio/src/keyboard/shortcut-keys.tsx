 ...
 * Delete (or Backspace) removes the selection only when no field, slider,
 * dialog or menu has the key; the file shortcuts work everywhere.
 *
 * This is the only handler of these keys, in a browser and in Difracta
...
export function ShortcutKeys() {
  ...
  const { remove } = useRemoveSelection();
  useEffect(() => {
    const commands = { ...documentCommands, remove };
    function onKeyDown(event: KeyboardEvent): void {
      ...
      if (name === "remove" && keyBelongsElsewhere(event)) return;
